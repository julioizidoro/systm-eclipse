...
@Named
@ViewScoped
public class CidadesVendidasMB implements Serializable {
	...
	public void setNumeroCidades(int numeroCidades) {
	...
	public void gerarRelatorio() throws IOException, SQLException {
		...
		sql = sql + " and vendas.dataVenda>='" + Formatacao.ConvercaoDataSql(dataInicio) + "' and vendas.dataVenda<='"
				...
				+ " group by cidade.idcidade order by count(vendas.idvendas) DESC";
		if (numeroCidades > 0) {
			sql = sql + " limit " + numeroCidades;
		}
		parameters.put("sql", sql);
		GerarRelatorio gerarRelatorio = new GerarRelatorio();
		try {
			gerarRelatorio.gerarRelatorioSqlPDF(caminhoRelatorio, parameters, "Cidades Vendidas.pdf", null);
		} catch (JRException ex1) {
			Logger.getLogger(CidadesVendidasMB.class.getName()).log(Level.SEVERE, null, ex1);
		} catch (IOException ex) {
			Logger.getLogger(CidadesVendidasMB.class.getName()).log(Level.SEVERE, null, ex);
		}
	}
}
