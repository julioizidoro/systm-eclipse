...
public class FaturaFranquiaBean {
	...
	private boolean cursoPacote;

	public FaturaFranquiaBean(Vendascomissao vendascomissao, List<Parcelamentopagamento> listaParcelamento,
			Float percentualComissao, AplicacaoMB aplicacaoMB, boolean seguroAvulso, boolean cursoPacote)
			throws SQLException {
		this.cursoPacote = cursoPacote;
	...
	public void gerarFaturaFranquias(List<Parcelamentopagamento> listaParcelamento, Vendascomissao vendascomissao)
			throws SQLException {
		Float liquido = 0.0f;
		...
		liquido = liquido - (custoFinanceiroFranquia + descontoMatriz + fatura.getVendascomissao().getDescontoloja()
				+ valorTotal);
	...
	public void calularTotaprodutos() {
		if (fatura.getVendascomissao().getVendas().getOrcamento() != null) {
			if (fatura.getVendascomissao().getVendas().getOrcamento().getOrcamentoprodutosorcamentoList() != null) {
				fatura.setTotalprodutos(somar(
						fatura.getVendascomissao().getVendas().getOrcamento().getOrcamentoprodutosorcamentoList()));
	...
	public void calcularMesAnoFatura(Vendascomissao vendasComissao) {
		...
		if (fatura.getLiquidopagar() >= 0) {
			...
			} else if ((vendasComissao.getVendas().getProdutos().getTipo().equalsIgnoreCase("P"))
					&& (vendasComissao.getVendas().getProdutos().getIdprodutos() != 2)) {
				mes = Formatacao.getMesData(vendasComissao.getVendas().getDataVenda()) + 1;
				ano = Formatacao.getAnoData(vendasComissao.getVendas().getDataVenda());
				if (mes == 12) {
					mes = 1;
